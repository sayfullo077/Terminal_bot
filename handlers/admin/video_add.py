...
from states.my_states import VideoAdd
...
@dp.message(F.text == '🎥 Video', IsBotOrAssistantAdmin())
async def get_format_video(message: types.Message, state: FSMContext):
    back_btn = await back_button()
    await message.answer(html.bold("Post videosini yuboring."),
                         reply_markup=back_btn)
    await state.set_state(VideoAdd.video)

#
# @dp.message(VideoAdd.video, IsBotOrAssistantAdmin())
# async def get_video(message: types.Message, session: AsyncSession, state: FSMContext):
...
#         await state.set_state(VideoAdd.url)
#
#     else:
#         await message.answer(html.bold("Post videosini yuboring!" if language == 'uz' else "Отправьте видео пост!"))
#         await state.set_state(VideoAdd.video)
#
#
# @dp.message(F.text.in_(["Davom etish ➡️", "Продолжить ➡️"]), VideoAdd.url, IsBotOrAssistantAdmin())
# async def skip_text_video(message: types.Message, session: AsyncSession, state: FSMContext):
#     telegram_id = message.from_user.id
#     user = await select_user(telegram_id, session)
#     language = user.languages
...
#     await state.set_state(VideoAdd.send)
#
#
# @dp.message(VideoAdd.url, IsBotOrAssistantAdmin())
# async def get_video_send(message: types.Message, session: AsyncSession, state: FSMContext):
#     telegram_id = message.from_user.id
#     user = await select_user(telegram_id, session)
#     language = user.languages
#     send_btn = await send_button(language)
...
#
#
# @dp.message(F.text.in_(["📤 Yuborish", "📤 Отправить"]), VideoAdd.send, IsBotOrAssistantAdmin())
# async def send_add(message: types.Message,  session: AsyncSession, state: FSMContext):
#     await state.set_state(VideoAdd.check)
#     user_id = message.from_user.id
#     user = await select_user(telegram_id=user_id, session=session)
#     language = user.languages
#     text = "Kimlar uchun video xabarini yubormoqchisiz?" if language == 'uz' else "Кому вы хотите отправить фотосообщение?"
#     send_btn = await ad_send_btn(language)
#     await message.answer(text=text, reply_markup=send_btn)
#
#
# @dp.message(F.text.in_(["Buyurtma berganlar ✅", "Те, кто сделали заказ ✅"]), VideoAdd.check, IsBotOrAssistantAdmin())
...
#             for i in users:
#                 try:
#                     await bot.send_video(video=data['video'], caption=data['caption'], chat_id=i.user_id,
...
#
#
# @dp.message(F.text.in_(["Buyurtma bermaganlar ❌", "Те, кто не заказывал ❌"]), VideoAdd.check, IsBotOrAssistantAdmin())
...
#
#
# @dp.message(F.text.in_(["Hamma uchun 👨‍👦‍👦", "Для всех 👨‍👦‍👦"]), VideoAdd.check, IsBotOrAssistantAdmin())
